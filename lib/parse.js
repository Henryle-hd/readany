...
export async function parseDoc(name) {
  ...
  const out = {
    name, source,
    pages: pages.map(paras => ({ paras: paras.map(p => toSentences(p.replace(INVISIBLE, '').replace(/\s+/g, ' ').trim())).filter(s => s.length) })),
  }
...
// zero-width / direction marks / BOM: PDFs often emit these for bullets and they render as empty lines
const INVISIBLE = /[\u200b-\u200f\u2028\u2029\u202a-\u202e\u2060-\u2064\ufeff\u00ad]/g
...
function toSentences(para) {
  ...
  return out.flatMap(s => splitLong(s)).filter(s => SPEAKABLE.test(s))
}
